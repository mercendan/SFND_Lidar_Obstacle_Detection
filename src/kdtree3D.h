...
struct KdTree
{
	...
	void searchHelper(std::vector<float> target, Node* node, int depth, float distanceTol, std::vector<int>& ids)
	{
		if (node != NULL)
		{
			...
			{
				float distanceXY = sqrt((node->point[0] - target[0]) * (node->point[0] - target[0]) + (node->point[1] - target[1]) * (node->point[1] - target[1]));
				float distance = sqrt(distanceXY * distanceXY + (node->point[02] - target[2]) * (node->point[2] - target[2]));
